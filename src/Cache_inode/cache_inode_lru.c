          ...
          if (atomic_fetch_size_t(&open_fd_count)
              < lru_state.fds_lowat) {
               ...
               size_t worktodoperlane=0;
                   ...
                   LogFullDebug(COMPONENT_CACHE_INODE_LRU,
                                "rate:%d = fdcount:%d - prev:%d / curr:%d prev:%d",
                                ratepersec, open_fd_count, prev_open_fd_count, curr_time, prev_lru_time);

               /* SLOW FD USE, slowly free below low water mark. */
               if (ratepersec <= 0)
                 {
                   LogFullDebug(COMPONENT_CACHE_INODE_LRU,
                   ...
                   if (worktodo > 0)
                     worktodoperlane = (worktodo /
                                        cache_inode_gc_policy.lru_run_interval) / 10;
                   else
                     {
                       worktodo=0;
                       worktodoperlane=0;
               ...
               /* FAST FD USE, free to low water mark */
               else
                 {
                    ...
                    * gc interval we _cannot_ keep up.  */
                   ...
                   LogFullDebug(COMPONENT_CACHE_INODE_LRU,
                                "FD rate per second is fast. threadwait:%lu ms", threadwait);
                   ...
                   LogFullDebug(COMPONENT_CACHE_INODE_LRU,
                                "ratepersec=%zd lru_run_interval=%d overlow %ld worktodo=%ld",
                                ratepersec, cache_inode_gc_policy.lru_run_interval, overlow,
                                worktodo);
                   worktodoperlane = worktodo/LRU_N_Q_LANES;

                   LogDebug(COMPONENT_CACHE_INODE_LRU,
                            "worktodo=%zd  ratepersec=%zd lru_run_interval=%u"
                            " overlow=%zd", worktodo, ratepersec,
                            cache_inode_gc_policy.lru_run_interval, overlow);
                 }
                    ...
                    for (lane = 0; lane < LRU_N_Q_LANES; ++lane) {
                         ...
                         while ((workdone < worktodoperlane) &&
                                (lru = glist_first_entry(&LRU_1[lane].lru.q,
                                                         cache_inode_lru_t,
                                                         q)) &&
                                (formeropen - closed > lru_state.fds_lowat)) {
